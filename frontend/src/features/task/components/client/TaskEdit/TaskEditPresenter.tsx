...
import type { TaskEditFormData } from "./schema";
import {
  DndContext,
  closestCenter,
  KeyboardSensor,
...
interface TaskEditPresenterProps {
  register: UseFormRegister<TaskEditFormData>;
  control: Control<TaskEditFormData>;
  fields: Array<{
    id: string;
  ...
  errors: FieldErrors<TaskEditFormData>;
...
export function TaskEditPresenter({
  register,
  control,
  fields,
  errors,
    ...
    <form onSubmit={onSubmit} className="bg-white border border-gray-200 rounded-lg p-6">
      ...
      <div className="relative flex items-center justify-between mb-6">
        ...
        <h1 className="text-2xl font-semibold absolute left-1/2 transform -translate-x-1/2">
          {date ? formatDateForDisplay(date) : ""}のタスク編集
        </h1>
        <Button
          type="button"
          variant="destructive"
      ...
      <div className="flex justify-center">
        <Button
          ...
          className="bg-black text-white hover:bg-gray-800 rounded-full px-8 py-2 disabled:opacity-50"
        >
          {isSubmitting ? "更新中..." : "更新"}
        </Button>
      </div>
