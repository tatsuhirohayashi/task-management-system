...
/**
 * TaskItem API Route
 * PUT /api/taskitems/:id - 子タスクのアウトプット更新
 */
export async function PUT(
  ...
  try {
    ...
    const result = await updateTaskItemCommand(id, ownerId, body);
    ...
    if (error instanceof Error) {
      if (error.message === "Task not found" || error.message === "Task item not found") {
        return NextResponse.json(
          { error: error.message },
          { status: 404 },
        );
